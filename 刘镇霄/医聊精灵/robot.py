...
class Prompts:
    ...
    # 欢迎提示词
    greeting_prompt = "你好[我是Molly]医疗精灵[专注解决你的医疗问题。请问你需要什么帮助]"
    # 对话提示词模板
...
class Robot:
    def __init__(self, model_config, retriever=None):
        ...
        llm_hist = RunnableWithMessageHistory(
            ...
            history_messages_key="chat_history"  # chain被invoke时，传入的chat_history会被存储在这个key中
        )
        self.chain = {
    ...
    def check_session_id(self):
        ...
        con = sqlite3.connect("files/chat_history.db")
        cursor = con.cursor()
    ...
    def get_history(self, session_id):
        if session_id not in self.check_session_id():
            history = SQLChatMessageHistory(session_id, "sqlite:///files/chat_history.db")
        ...
        return SQLChatMessageHistory(session_id, "sqlite:///files/chat_history.db")
...
if __name__ == "__main__":
    ...
    result = robot.chat(input='你能帮我找附近的安全吗？', session_id='abc789')
    print('答复！', result)