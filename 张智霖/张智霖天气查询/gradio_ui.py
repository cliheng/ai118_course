...
def update_model_name(model_input):
    # 根据选择的大模型平台更新模型名称
    return funtion_tools.MODEL_MAPPING[model_input]["model_name"]

def chatbot_interface(query, model_input, temperature, max_tokens):
    # 处理聊天请求
    config = funtion_tools.MODEL_MAPPING[model_input]
    ...
    if response.choices[0].message.tool_calls is not None:
        ...
        invoke_fun = getattr(funtion_tools, function_name)
        ...
        with gr.Row():
            ...
            with gr.Column(scale=1):
                ...
                model_input = gr.Radio(
                    list(funtion_tools.MODEL_MAPPING.keys()),
                    label="",
                    value="Openai"
                )
                gr.Markdown("### 模型名称")
                model_name = gr.Textbox(
                    label="",
                    value=funtion_tools.MODEL_MAPPING["Openai"]["model_name"],
                    interactive=False
                )
