...
def llm_chat(message):
    try:
        ...
        if not api_key:
            # 如果环境变量不存在，使用默认值
            api_key = "21862f76d8734213a46b15cfd2c328f6.KmNuT8f0GCWJhoPw"
            print("警告：使用默认API KEY")
...
if __name__ == "__main__":
    ...
    api_key = os.getenv('ZHIPUAI_API_KEY')
    if not api_key:
        print("警告：未找到 ZHIPUAI_API_KEY 环境变量，将使用默认值")
    else:
        print("成功加载 API KEY")
    ...
    try:
        # 尝试获取已存在的collection
        collection = client.get_collection("my_collection")
        print("成功加载已存在的collection")
    except Exception as e:
        print(f"Collection不存在，开始创建新的collection: {e}")
        # 创建新的collection并导入数据
        keywords, contents = parse_json('data_source.json')
        embeddings = load_embeddings(keywords)
        collection = client.create_collection("my_collection")
