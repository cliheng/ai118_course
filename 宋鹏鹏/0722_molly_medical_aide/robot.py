...
class Robot:
    ...
    def __init__(self,model_config,retriever=None):
        ...
        llm_hist = RunnableWithMessageHistory(
            ...
            history_messages_key="chat_history"   # chain被invoke时，传入的chat_history会被储存在这个key中
        )
    ...
    def get_history(self,session_id):
        existing_sessions = self.check_session_id() or []
        if session_id not in existing_sessions:
            history = SQLChatMessageHistory(session_id, "sqlite:///chat_history.db")
            # 如果session_id不存在，先添加系统提示和欢迎提示
...
if __name__ == "__main__":
    ...
    result = robot.chat(input="我需要咨询一下关于脑卒中的问题", session_id="abc123")
    print("答复：",result)