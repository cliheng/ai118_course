...
class Robot:
    def __init__(self, model_config, retriever=None):
        ...
        self.chain = {'input': RunnablePassthrough(), 'rag_results': retriever,
                      'chat_history': RunnablePassthrough()} | llm_hist
    ...
    def get_history(self, session_id):
        if session_id not in self.check_session_id():
            # 如果session_id不存在，则创建新表存储提示和欢迎提示
...
if __name__ == "__main__":
    ...
    result = robot.chat("我肠胃不太好，医生建议我吃药膳调理，请告诉我北京有哪些地方可以吃药膳", session_id='abc789')
    print("答复：", result)
